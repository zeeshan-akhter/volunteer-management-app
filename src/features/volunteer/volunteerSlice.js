...
export const fetchVolunteers = createAsyncThunk('/volunteers/fetchVolunteers', 
  async () => {
    ...
    const response = await axios.get('https://volunteermanagementapi.zeeshanakhter.repl.co/volunteer');
    return response?.data?.data;
  }
);

export const addVolunteer = createAsyncThunk('/volunteers/addVolunteer', async (bodyData) => {
  const response = await axios.post('https://volunteermanagementapi.zeeshanakhter.repl.co/volunteer', bodyData);
  toast.success(response.data.message);
  return response.data.data;
});

export const deleteVolunteer = createAsyncThunk('/volunteers/deleteVolunteer', async (id) => {
  const response = await axios.delete(`https://volunteermanagementapi.zeeshanakhter.repl.co/volunteer/${id}`);
  toast.success(response.data.message);
  return response.data.data;
});

export const updateVolunteer = createAsyncThunk('/volunteers/updateVolunteer', async ({ id, volunteerData }) => {
  const response = await axios.put(`https://volunteermanagementapi.zeeshanakhter.repl.co/volunteer/${id}`, volunteerData);
  toast.success(response.data.message);
